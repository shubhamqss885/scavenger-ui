...
// Local modification (Scavenger): added optional `stream` prop so a parent
// (useAudioRecording hook) can pass in an existing MediaStream instead of
// the component opening its own getUserMedia. Avoids double mic capture
// during dictation. If `stream` is omitted the component falls back to its
...
export type LiveWaveformProps = HTMLAttributes<HTMLDivElement> & {
  ...
  processing?: boolean;
  deviceId?: string;
  /** Externally-managed stream. When provided, the component uses it as
   *  the analyser source and does NOT call getUserMedia or stop tracks. */
...
export const LiveWaveform = ({
  ...
  // True only when this component opened the stream itself; false when the
  // caller provided one via the `stream` prop. Used so cleanup doesn't stop
  // tracks the caller still owns.
  ...
  // Two paths:
  //   1) `externalStream` provided → use it directly; do NOT call
  //      getUserMedia and do NOT stop tracks on cleanup (caller owns it).
  //   2) No external stream → original self-managed behavior.
  useEffect(() => {
    ...
    const attachAnalyser = (mediaStream: MediaStream) => {
    ...
    if (externalStream) {
      ...
      ownsStreamRef.current = false;
      try {
        attachAnalyser(externalStream);
      } catch (error) {
        onError?.(error as Error);
      }

      return () => {
        // Caller owns the stream — only release our analyser.
        releaseAnalyser();
        streamRef.current = null;
      };
    }
    ...
    const setupMicrophone = async () => {
      try {
        ...
        onStreamReady?.(acquired);
        attachAnalyser(acquired);
      } catch (error) {
        onError?.(error as Error);
      }
    };
